...
function addDestination(dest: string) {
  ...
  const editTrip = async (): Promise<void> => {
    try {
      ...
      // Prepare the request body with all the form data
      const requestBody = {
        
        name: formData2.name,
        startDate: formatDisplayDateToApi(formData.startDate), // Convert back to API format
        endDate: formatDisplayDateToApi(formData.endDate),
        destinations: selected,
        maxParticipants: maxParticipant,
        pricePerPerson: pricePerPerson,
        includedServices: selectedServices.map(serviceId => 
          services.find(service => service.id === serviceId)?.title
        ).filter(Boolean), // Convert IDs back to titles
        detail: editorState.content,
        travelStyles: selectedItems.map(categoryId => 
      ...
      // Navigate back or to trip details
      // router.back(); or router.push('/TripDetails');
